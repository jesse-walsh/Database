...
ALTER TABLE CSVImport ADD COLUMN ref_date DATE;
...
LOAD DATA LOCAL INFILE "C:/Users/Jesse/Documents/GitHub/Database/waitlistdata.csv"
...
INSERT IGNORE INTO Surgeon (
	surgeon_fName,
    surgeon_lName,
    dept_id
) SELECT DISTINCT
	SUBSTRING(surgeon_fullName, 1, LOCATE(' ', surgeon_fullName) - 1) AS surgeon_fName,
	REVERSE(SUBSTRING(REVERSE(surgeon_fullName), 1, LOCATE(' ', REVERSE(surgeon_fullName)) - 1)) AS surgeon_lName,
	Department.dept_id FROM CSVImport 
	INNER JOIN Department ON Department.dept_name = CSVImport.dept_name;
...
INSERT INTO Referral (
	...
	hte,
	nhi,
	Surgeon.surgeon_id FROM CSVImport
    INNER JOIN Surgeon ON CONCAT(Surgeon.surgeon_fName, " ", Surgeon.surgeon_lName) = CSVImport.surgeon_fullName;